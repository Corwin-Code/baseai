...
import com.cloud.baseai.infrastructure.external.llm.impl.MockEmbeddingService;
import com.cloud.baseai.infrastructure.external.llm.impl.OpenAIEmbeddingService;
import com.cloud.baseai.infrastructure.external.llm.impl.QianWenEmbeddingService;
...
public class EmbeddingModuleAutoConfiguration {

    private static final Logger log = LoggerFactory.getLogger(EmbeddingModuleAutoConfiguration.class);
    ...
    public EmbeddingService openaiEmbeddingService(EmbeddingProperties config) {
    ...
    public EmbeddingService qianwenEmbeddingService(EmbeddingProperties config) {
     ...
     * 验证OpenAI配置
     */
    private void validateOpenAIConfig(EmbeddingProperties config) {
     ...
     * 验证千问配置
     */
    private void validateQianWenConfig(EmbeddingProperties config) {
        if (config.getApiKey() == null || config.getApiKey().trim().isEmpty()) {
